<!-- Generated: 2026-05-04 | Files scanned: ~250 | Token estimate: ~600 -->
...
User actions:
  place bet     → place_bet RPC → bets + balance_transactions + balances
  market close  → close_expired_markets() (every minute, see migration 045)
  market resolve → settle_market RPC → bets.status = won|lost + payout transfer
```
...
src/entities  (domain primitives: balance, bet, manager, market, outcome, transaction, user)
