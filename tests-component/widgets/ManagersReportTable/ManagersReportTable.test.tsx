...
describe('ManagersReportTable', () => {
  it('renders each manager row with deposits, withdrawals and signed profit', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Manager One')).toBeInTheDocument());

    const rows = screen.getAllByRole('row');
    // header + 2 body rows + footer total row
    expect(rows).toHaveLength(4);

    const row1 = within(screen.getByText('Manager One').closest('tr') as HTMLElement);
    ...
    const row2 = within(screen.getByText('Manager Two').closest('tr') as HTMLElement);
  ...
  it('coerces string numerics from the RPC and formats them to two decimals', async () => {
    ...
    const bodyRow = within(screen.getByText('Manager One').closest('tr') as HTMLElement);
    ...
    const totalRow = within(screen.getByText('Total').closest('tr') as HTMLElement);
  ...
  it('falls back to @username when the manager has no full name', async () => {
    ...
    await waitFor(() => expect(screen.getByText('@mgr_one')).toBeInTheDocument());
  });
