...
const COLOR_HEADER = rgb(0.15, 0.15, 0.2);
  ...
  for (let ri = 0; ri < rows.length; ri++) {
    ...
    for (let ci = 0; ci < cellLines.length; ci++) {
      ...
      for (let li = 0; li < cellLines[ci].length; li++) {
        const lineText = cellLines[ci][li];
        const textW = ctx.font.widthOfTextAtSize(lineText, CELL_SIZE);
        drawText(
          page,
          toVisual(lineText),
          cellRight - CELL_PAD - textW,
          y - ROW_PAD / 2 - (li + 1) * LINE_H + 4,
          ctx.font,
          CELL_SIZE
        );
      }
...
function drawKpisRTL(
  ...
  drawText(
    page,
    toVisual(indText),
    rightEdge - CELL_PAD - font.widthOfTextAtSize(indText, LABEL_SIZE),
    y - HEADER_H + 8,
    font,
    LABEL_SIZE,
    COLOR_WHITE
  ...
  drawText(
    page,
    toVisual(valueText),
    rightEdge - labelW - CELL_PAD - font.widthOfTextAtSize(valueText, LABEL_SIZE),
    y - HEADER_H + 8,
    font,
    LABEL_SIZE,
    COLOR_WHITE
  ...
  for (let i = 0; i < kpis.length; i++) {
    ...
    const valueText = kpis[i].value;
    drawText(
      page,
      toVisual(labelText),
      rightEdge - CELL_PAD - font.widthOfTextAtSize(labelText, CELL_SIZE),
      y - rowH + ROW_PAD / 2,
      font,
      CELL_SIZE
    );
    drawText(
      page,
      toVisual(valueText),
      rightEdge - labelW - CELL_PAD - font.widthOfTextAtSize(valueText, CELL_SIZE),
      y - rowH + ROW_PAD / 2,
      font,
      CELL_SIZE
    );
...
export async function renderTablePdf(doc: ReportDocument): Promise<Uint8Array> {
  ...
  if (isRTL) {
    ...
    drawText(firstPage, toVisual(titleText), rightEdge - titleW, y, font, TITLE_SIZE);
    y -= 22;
    ...
    drawText(
      firstPage,
      toVisual(periodLabel),
      rightEdge - periodLabelW,
      y,
      font,
      LABEL_SIZE,
      COLOR_GRAY
    ...
    drawText(
      firstPage,
      toVisual(periodValue),
      rightEdge - periodLabelW - periodValueW,
      y,
      font,
      LABEL_SIZE,
      COLOR_GRAY
    ...
    drawText(firstPage, toVisual(genLabel), rightEdge - genLabelW, y, font, CELL_SIZE, COLOR_GRAY);
    drawText(
      firstPage,
      toVisual(genValue),
      rightEdge - genLabelW - genValueW,
      y,
      font,
      CELL_SIZE,
      COLOR_GRAY
