-- Migration: raise the order-book freshness bound from 5s to 30s
--
-- WHY. place_bet / sell_position rejected with "Market book is stale" whenever
-- market_outcome_books.book_updated_at was older than 5 seconds. The book is
-- kept warm by (a) the client polling quote-bet every 2s (each call re-UPSERTs
-- the book with updated_at = now()) and (b) the market-tracker WS flush (~1s,
-- only for subscribed tokens). A 5s bound is brittle against that 2s poll: a
-- single slow CLOB fetch (quote-bet's own timeout is 4s), a backgrounded tab
-- that pauses the poll (refetchIntervalInBackground:false), or any network
-- jitter pushes the book past 5s and the user gets a stale rejection on a
-- perfectly legitimate bet.
--
-- 30s is NOT a weaker antifraud posture. The displayed bet-page price already
-- refreshes only every 30s (MARKETS_REFRESH_INTERVAL_MS) — that 30s window is
-- the documented antifraud bound. The 5s book bound was a technical guard
-- against executing on a torn book, not the antifraud window. Aligning the book
-- bound to the same 30s as the price the user actually sees keeps the system no
-- looser than its weakest link, while the 2% effective-odds / sell-price drift
-- guard still rejects any bet whose price has actually moved beyond tolerance.
--
-- Functions are redefined verbatim from 20260602130123 (place_bet) and
-- 20260602130622 (sell_position); the ONLY change is the c_book_max_staleness
-- constant (5s -> 30s) and the corresponding COMMENT text. The 3-minute price-
-- feed bound (c_max_staleness) is unchanged.
...
-- CREATE OR REPLACE preserves the existing ACL, but re-issue the grants to keep
-- this migration self-contained and to mirror 20260602130622 exactly.
...
COMMENT ON FUNCTION sell_position(uuid, numeric, numeric) IS
  'SELL (early exit) in the positions/trades model. Sells p_shares of the caller''s own open position at the slippage-adjusted bid price, crediting proceeds to available, releasing the sold cost basis from in_play, crystallizing realized P/L into the position and a sell trade + bet_sell ledger row. Requires a fresh (<=30s) non-partial bid quote on an open+visible market. Rejects partial fills (resize to filled_shares). 2% price-drift guard (ERRCODE P0002).';
